...
public:
    vector<int> findAllPeople(int n, vector<vector<int>> &meetings, int firstPerson)
    {
        ...
        {
            while (r < MEETINGS_NUM && meetings[r + 1][2] == meetings[l][2])
                ++r;
            for (int i = l; i <= r; ++i)
                unionn(meetings[i][0], meetings[i][1], father);
            for (int i = l; i <= r; ++i)
            {
                int a = meetings[i][0], b = meetings[i][1];
                if (!isSameSet(0, a, father))
                    father[a] = a;
                if (!isSameSet(0, b, father))
                    father[b] = b;
            }
            l = ++r;
        }
